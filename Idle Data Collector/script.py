...
idle_duration = float(sender_config['Duration'])
...
def fresh_start(freq):
    ...
    path = f"{folder_name}/"
    file_name = f"{path}{freq}_idle.iq"
...
def sniffPackets(freq, pcap_path):
    ...
    wrpcap(f'{pcap_path}/{freq}Mhz_sniffed_idle.pcap', pckts)
...
def main():
    for freq in range(start_freq, end_freq+1, step):
        ...
        thread_sender = threading.Thread(target=sendPackets)
